...
- `enable_booster_flag`：允许 Booster 执行校准、发射状态机；为 0 时 Push/Pull 停机，Reload 角度电机保持安全初始角。
...
当前实际发射流程集中在 `Class_FSM_Shooting::Shooting_TIM_Status_PeriodElapsedCallback()`，换弹动作也被整合进这个大状态机里。`Class_FSM_Reload` 仍保留类和枚举，但当前主流程没有单独调度它。
...
2. 如果要恢复拉力闭环发射准备，需要重新启用 PREP_FIRE 中的 `Pull_Tension_Control()` 分支，并确认 `Target_Tension`、拉力计读数单位、稳定时间阈值是否一致。
3. `Referee_Allow_Shoot` 同时影响 Booster 和 Gimbal。改它的置位/清零时，要同时检查发射触发和 Yaw 冻结逻辑。
4. `Motor_Pitch_L/R` 虽然当前不输出 PID，但外部仍会做 CAN 接收和 Alive 检测，不能简单删除。
5. `Control_Gimbal()` 中当前 `tmp_gimbal_pitch` 的读取逻辑需要后续确认：代码会写 `Set_Target_Pitch_Angle(tmp_gimbal_pitch)`，但当前 Pitch 没有实际输出控制。
6. `make -j4` 当前在本仓库根目录报 `No targets`，实际编译建议继续使用 Keil / MDK 工程或补齐 Makefile 规则后再验证。
